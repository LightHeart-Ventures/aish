...
svc_count=0
[ ${#services[@]} -gt 0 ] 2>/dev/null && svc_count=${#services[@]} || svc_count=0
if [ "$svc_count" -eq 0 ] && [ -n "${SIGNOZ_WATCH_SERVICES:-}" ]; then
  IFS=',' read -r -a services <<< "$SIGNOZ_WATCH_SERVICES"
  svc_count=${#services[@]}
fi
[ "$svc_count" -eq 0 ] && exit 0   # nothing instrumented here — quiet exit
